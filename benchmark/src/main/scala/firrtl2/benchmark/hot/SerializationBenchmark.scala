...
object SerializationBenchmark extends App {
  ...
  if(select == "n") {
    ...
    firrtl2.benchmark.hot.util.benchmark(warmup, runs)(Serializer.serialize(input))
  } else if(select == "o") {
    println("Benchmarking legacy serialization")
    firrtl2.benchmark.hot.util.benchmark(warmup, runs)(input.serialize)
