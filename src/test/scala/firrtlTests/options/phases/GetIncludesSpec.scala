...
class GetIncludesSpec extends AnyFlatSpec with Matchers with firrtl.testutils.Utils {
  ...
  val files = Seq(
    new File(dir.getAbsolutePath + "/a.anno.json") -> Seq(A, ref("b")),
    new File(dir.getAbsolutePath + "/b.anno.json") -> Seq(B, ref("c"), ref("a")),
    new File(dir.getAbsolutePath + "/c.anno.json") -> Seq(C, ref("d"), ref("e")),
    new File(dir.getAbsolutePath + "/d.anno.json") -> Seq(D),
    new File(dir.getAbsolutePath + "/e.anno.json") -> Seq(E)
  )
