...
class DedupModuleTests extends HighFirrtlTransformSpec {
  ...
  "Modules with aggregate ports that are connected" should "NOT dedup if their port names differ" in {
    val input =
      """
  ...
  "Modules with aggregate ports that are connected" should "dedup if their port names are the same" in {
    val input =
      """
        |circuit FooAndBarModule :
        |  module FooModule :
