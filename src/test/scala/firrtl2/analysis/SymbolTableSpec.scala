...
package firrtl2.analysis

import firrtl2.analyses._
import firrtl2.ir
...
class SymbolTableSpec extends AnyFlatSpec {
  ...
  it should "find all declarations in module m before InferTypes" in {
    val c = firrtl2.Parser.parse(src)
    val m = c.modules.find(_.name == "m").get

    val syms = SymbolTable.scanModule(m, new LocalSymbolTable with WithMap)
    assert(syms.size == 8)
    assert(syms("clk").tpe == ir.ClockType && syms("clk").kind == firrtl2.PortKind)
    assert(syms("x").tpe == ir.UIntType(ir.IntWidth(1)) && syms("x").kind == firrtl2.PortKind)
    assert(syms("y").tpe == ir.UIntType(ir.IntWidth(3)) && syms("y").kind == firrtl2.PortKind)
    assert(syms("z").tpe == ir.SIntType(ir.IntWidth(1)) && syms("z").kind == firrtl2.WireKind)
    // The expression type which determines the node type is only known after InferTypes.
    assert(syms("a").tpe == ir.UnknownType && syms("a").kind == firrtl2.NodeKind)
    ...
    assert(syms("i").tpe == ir.UnknownType && syms("i").kind == firrtl2.InstanceKind)
    assert(syms("r").tpe == ir.SIntType(ir.IntWidth(4)) && syms("r").kind == firrtl2.RegKind)
    val mType = firrtl2.passes.MemPortUtils.memType(
    ...
    assert(syms("m").tpe == mType && syms("m").kind == firrtl2.MemKind)
  }

  it should "find all declarations in module m after InferTypes" in {
    val c = firrtl2.Parser.parse(src)
    val inferTypesCompiler = new firrtl2.stage.TransformManager(Seq(Dependency(firrtl2.passes.InferTypes)))
    val inferredC = inferTypesCompiler.execute(firrtl2.CircuitState(c, Seq())).circuit
    val m = inferredC.modules.find(_.name == "m").get

    val syms = SymbolTable.scanModule(m, new LocalSymbolTable with WithMap)
    // The node type is now known
    assert(syms("a").tpe == ir.UIntType(ir.IntWidth(2)) && syms("a").kind == firrtl2.NodeKind)
    ...
    assert(syms("i").tpe == iType && syms("i").kind == firrtl2.InstanceKind)
  }
  ...
  it should "derive the module type from the module types map" in {
    ...
    assert(syms.size == 8)
    assert(syms("i").tpe == childType && syms("i").kind == firrtl2.InstanceKind)
  }
}
