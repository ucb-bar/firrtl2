  ...
  private def legacyLower(n: String, tpe: String, namespace: Set[String]): Seq[String] = {
    ...
    val src =
      s"""circuit c:
         ...
         |    $n is invalid
         |""".stripMargin
    val c = CircuitState(firrtl2.Parser.parse(src), Seq())
...
class LowerTypesOfInstancesSpec extends AnyFlatSpec with FirrtlMatchers {
  import LowerTypesSpecUtils._
  private case class Lower(inst: firrtl2.ir.DefInstance, fields: Seq[String], renameMap: RenameMap)
  private val m = CircuitTarget("m").module("m")
  ...
  def resultToFieldSeq(res: Seq[(String, firrtl2.ir.SubField)]): Seq[String] =
  ...
  private def lower(
    ...
    val ref = firrtl2.ir.DefInstance(firrtl2.ir.NoInfo, n, module, parseType(tpe))
  ...
  it should "lower an instance correctly" in {
    ...
    assert(l.inst.tpe.isInstanceOf[firrtl2.ir.BundleType])
  ...
  it should "update the rename map with the changed port names" in {
    ...
    // same as above but with lowered port
    {
      ...
      val portB = firrtl2.ir.Field("b", firrtl2.ir.Default, parseType("{ c : UInt<1>}"))
      val portB_C = firrtl2.ir.Field("b_c", firrtl2.ir.Default, parseType("UInt<1>"))
...
class LowerTypesOfMemorySpec extends AnyFlatSpec {
  ...
  private case class Lower(
    mems:      Seq[firrtl2.ir.DefMemory],
    refs:      Seq[(String, firrtl2.ir.SubField)],
    renameMap: RenameMap)
  private val m = CircuitTarget("m").module("m")
  ...
  private def lower(
    ...
    val mem = firrtl2.ir.DefMemory(
      ...
      writeLatency = 1,
      readLatency = 1,
      readUnderWrite = firrtl2.ir.ReadUnderWrite.Undefined,
      readers = r,
      writers = w,
      readwriters = rw
    )
  ...
  private val UInt1 = firrtl2.ir.UIntType(firrtl2.ir.IntWidth(1))
...
private object LowerTypesSpecUtils {
  ...
  def parseType(tpe: String): firrtl2.ir.Type = {
    val src =
      s"""circuit c:
         |  module c:
         |    input c: $tpe
  ...
  def destruct(n: String, tpe: String, namespace: Set[String]): DestructResult = {
    val ref = firrtl2.ir.Field(n, firrtl2.ir.Default, parseType(tpe))
    val renames = MutableRenameMap()
    val mutableSet = scala.collection.mutable.HashSet[String]() ++ namespace
    val res = DestructTypes.destruct(m, ref, mutableSet, renames, Set())
    DestructResult(resultToFieldSeq(res), renames)
  }
  def resultToFieldSeq(res: Seq[(firrtl2.ir.Field, String)]): Seq[String] =
