...
class StructuralHashSpec extends AnyFlatSpec {
  ...
  private val highFirrtlCompiler = new firrtl2.stage.transforms.Compiler(
    targets = firrtl2.stage.Forms.HighForm
  )
  private def parse(circuit: String): Circuit = {
    val rawFirrtl = firrtl2.Parser.parse(circuit)
    ...
    highFirrtlCompiler.transform(firrtl2.CircuitState(rawFirrtl, Seq())).circuit
  }
