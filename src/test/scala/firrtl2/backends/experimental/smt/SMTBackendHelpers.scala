...
import firrtl2.{ir, MemoryInitValue}
import firrtl2.stage.{Forms, RunFirrtlTransformAnnotation, TransformManager}
...
private object SMTBackendHelpers {
  ...
  def compile(src: String, annos: Seq[Annotation] = List()): ir.Circuit = {
    ...
    compiler.runTransform(firrtl2.CircuitState(c, annos)).circuit
  }

  def compileUndef(src: String, annos: Seq[Annotation] = List()): ir.Circuit = {
    val c = firrtl2.Parser.parse(src)
    undefCompiler.runTransform(firrtl2.CircuitState(c, annos)).circuit
  }
