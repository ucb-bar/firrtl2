...
import firrtl2.ir.{Field => _, _}
import firrtl2.Utils.{field_type, sub_type}
import AnnotationUtils.{toExp, validComponentName, validModuleName}
import TargetToken._

import scala.collection.mutable

/** Refers to something in a FIRRTL [[firrtl2.ir.Circuit]]. Used for Annotation targets.
  *
...
/** Target pointing to a FIRRTL [[firrtl2.ir.Circuit]]
  *
...
/** Target pointing to a FIRRTL [[firrtl2.ir.DefModule]]
  *
...
/** Target pointing to a declared named component in a [[firrtl2.ir.DefModule]]
  * This includes: [[firrtl2.ir.Port]], [[firrtl2.ir.DefWire]], [[firrtl2.ir.DefRegister]],
  * [[firrtl2.ir.DefMemory]], [[firrtl2.ir.DefNode]]
  *
...
case class ReferenceTarget(
  ...
  /** @return The initialization value of this reference, must be to a [[firrtl2.ir.DefRegister]] */
  def init: ReferenceTarget = ReferenceTarget(circuit, module, path, ref, component :+ Init)

  /** @return The reset signal of this reference, must be to a [[firrtl2.ir.DefRegister]] */
  def reset: ReferenceTarget = ReferenceTarget(circuit, module, path, ref, component :+ Reset)

  /** @return The clock signal of this reference, must be to a [[firrtl2.ir.DefRegister]] */
  ...
  def leafSubTargets(tpe: firrtl2.ir.Type): Seq[ReferenceTarget] = tpe match {
    case _: firrtl2.ir.GroundType => Vector(this)
    case firrtl2.ir.VectorType(t, size) => (0 until size).flatMap { i => index(i).leafSubTargets(t) }
    case firrtl2.ir.BundleType(fields)  => fields.flatMap { f => field(f.name).leafSubTargets(f.tpe) }
    case other                          => sys.error(s"Error! Unexpected type $other")
  }

  def allSubTargets(tpe: firrtl2.ir.Type): Seq[ReferenceTarget] = tpe match {
    case _: firrtl2.ir.GroundType => Vector(this)
    case firrtl2.ir.VectorType(t, size) => this +: (0 until size).flatMap { i => index(i).allSubTargets(t) }
    case firrtl2.ir.BundleType(fields)  => this +: fields.flatMap { f => field(f.name).allSubTargets(f.tpe) }
    case other                          => sys.error(s"Error! Unexpected type $other")
  }
