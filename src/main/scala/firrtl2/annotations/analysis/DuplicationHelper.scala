...
package firrtl2.annotations.analysis

import firrtl2.annotations._
import firrtl2.annotations.TargetToken.{Instance, OfModule, Ref}
import firrtl2.Utils.throwInternalError

import scala.collection.mutable

/** Used by [[firrtl2.annotations.transforms.EliminateTargetPaths]] to eliminate target paths
  ...
  def getModuleName(top: String, path: Seq[(Instance, OfModule)]): String = {
    cachedNames.get((top, path)) match {
      case None => // Need a new name
        ...
        val finalName = firrtl2.Namespace.findValidPrefix(prefix, Seq(postfix), ns) + postfix
