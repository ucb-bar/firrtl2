...
import firrtl2.Utils.{field_type, max, module_type, sub_type, throwInternalError}
...
object ConvertFixedToSInt extends Pass {
  ...
  def run(c: Circuit): Circuit = {
    ...
    (firrtl2.passes.InferTypes).run(Circuit(c.info, newModules.map(onModule(_)), c.main))
  }
}
