...
import firrtl2.analyses.{InstanceKeyGraph, SymbolTable}
...
object LowerTypes extends Transform {
  ...
  /** Expands a chain of referential [[firrtl2.ir.Expression]]s into the equivalent lowered name
    *
    * @param e [[firrtl2.ir.Expression]] made up of _only_ [[firrtl2.WRef]], [[firrtl2.WSubField]], and [[firrtl2.WSubIndex]]
  ...
  private def onStatement(
    ...
    case d: DefWire =>
      Block(
        symbols.lower(d.name, d.tpe, firrtl2.WireKind).map { case (name, tpe, _) => d.copy(name = name, tpe = tpe) }
      )
    case d @ DefRegister(info, _, _, clock, reset, _) =>
      ...
      val loweredRegs = symbols.lower(d.name, d.tpe, firrtl2.RegKind)
    ...
    case d: DefNode =>
      ...
      Block(symbols.lower(d.name, d.value.tpe, firrtl2.NodeKind).zip(values).map {
