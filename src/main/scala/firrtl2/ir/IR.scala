...
/** Use the [[firrtl2.ir.Serializer]] to serialize this node. */
private[firrtl2] trait UseSerializer extends FirrtlNode {
...
object MultiInfo {
  ...
  private[firrtl2] def demux(info: Info): (Info, Info, Info) = info match {
...
case class Verification(
  ...
  def mapType(f:     Type => Type):      Statement = this
  def mapString(f:   String => String):  Statement = copy(name = f(name))
