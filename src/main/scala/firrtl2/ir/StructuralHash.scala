...
object StructuralHash {
  ...
  private[firrtl2] def sha256(str: String): HashCode = {
    val m = MessageDigest.getInstance(SHA256)
  ...
  /** Using this to hash arbitrary nodes can have unexpected results like:
    ...
    */
  private[firrtl2] def sha256Node(node: FirrtlNode): HashCode = {
    val m = MessageDigest.getInstance(SHA256)
...
class StructuralHash private (h: Hasher, renameModule: String => String) {
  ...
  private def hash(node: Expression): Unit = node match {
    ...
    case firrtl2.EmptyExpression => id(12)
    // VRandom is used in the Emitter
    case firrtl2.VRandom(width) => id(13); hash(width)
  ...
  private def hash(node: Statement): Unit = node match {
    ...
    case firrtl2.DescribedStmt(_, stmt) => hash(stmt)
    case DefMemory(
          _,
          name,
          dataType,
    ...
    case firrtl2.CDefMPort(_, name, _, mem, exps, direction) =>
    ...
    case firrtl2.passes.memlib.DefAnnotatedMemory(
          _,
          name,
          dataType,
          depth,
  ...
  private def hash(d: firrtl2.MPortDir): Unit = d match {
    case firrtl2.MInfer     => id(-70)
    case firrtl2.MRead      => id(-71)
    case firrtl2.MWrite     => id(-72)
    case firrtl2.MReadWrite => id(-73)
  }

  private def hash(c: firrtl2.constraint.Constraint): Unit = c match {
    case b: Bound => hash(b) /* uses ids -80 ... -84 */
    case firrtl2.constraint.IsAdd(known, maxs, mins, others) =>
    ...
    case firrtl2.constraint.IsFloor(child, dummyArg) => id(-86); hash(child); hash(dummyArg)
    case firrtl2.constraint.IsKnown(decimal)         => id(-87); hash(decimal)
    case firrtl2.constraint.IsNeg(child, dummyArg)   => id(-88); hash(child); hash(dummyArg)
    case firrtl2.constraint.IsPow(child, dummyArg)   => id(-89); hash(child); hash(dummyArg)
    case firrtl2.constraint.IsVar(str)               => id(-90); n(str)
  }
