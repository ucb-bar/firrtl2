...
object Serializer {
  ...
  private def s(node: Expression)(implicit b: StringBuilder, indent: Int): Unit = node match {
    ...
    case firrtl2.EmptyExpression => b ++= "EMPTY"
    case other                   => b ++= other.serialize // Handle user-defined nodes
  }
  ...
  private def s(node: Statement)(implicit b: StringBuilder, indent: Int): Unit = node match {
    ...
    // WIR
    case firrtl2.CDefMemory(info, name, tpe, size, seq, readUnderWrite) =>
      ...
      s(info)
    case firrtl2.CDefMPort(info, name, _, mem, exps, direction) =>
      b ++= direction.serialize; b ++= " mport "; b ++= name; b ++= " = "; b ++= mem
      b += '['; s(exps.head); b ++= "], "; s(exps(1)); s(info)
    case firrtl2.WDefInstanceConnector(info, name, module, tpe, portCons) =>
