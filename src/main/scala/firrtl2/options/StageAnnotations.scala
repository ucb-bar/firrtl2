...
/** Mix-in that lets an [[firrtl2.annotations.Annotation Annotation]] serialize itself to a file separate from the output
  * annotation file.
  *
  * This can be used as a mechanism to serialize an [[firrtl2.options.Unserializable Unserializable]] annotation or to
  * write ancillary collateral used by downstream tooling, e.g., a TCL script or an FPGA constraints file. Any
  * annotations using this mix-in will be serialized by the [[firrtl2.options.phases.WriteOutputAnnotations
  * WriteOutputAnnotations]] phase. This is one of the last phases common to all [[firrtl2.options.Stage Stages]] and
...
trait CustomFileEmission { this: Annotation =>
  ...
  /** Optionally, a sequence of annotations that will replace this annotation in the output annotation file.
    *
    * A non-empty implementation of this method is a mechanism for telling a downstream [[firrtl2.options.Stage Stage]]
...
/** If this [[firrtl2.annotations.Annotation Annotation]] exists in an [[firrtl2.AnnotationSeq AnnotationSeq]], then a
  * [[firrtl2.options.phases.WriteOutputAnnotations WriteOutputAnnotations]] will include
  * [[firrtl2.annotations.DeletedAnnotation DeletedAnnotation]]s when it writes to a file.
