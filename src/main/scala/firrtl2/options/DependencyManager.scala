...
import firrtl2.graph.{CyclicException, DiGraph}
...
/** A [[firrtl2.options.TransformLike TransformLike]] that resolves a linear ordering of dependencies based on
  * requirements.
  *
  * @tparam A the type over which this transforms
  * @tparam B the type of the [[firrtl2.options.TransformLike TransformLike]]
  */
trait DependencyManager[A, B <: TransformLike[A] with DependencyAPI[B]] extends TransformLike[A] with DependencyAPI[B] {
  ...
  /** Requested [[firrtl2.options.TransformLike TransformLike]]s that should be run. Internally, this will be converted to
  ...
  /** A sequence of [[firrtl2.Transform]]s that have been run. Internally, this will be converted to an ordered set.
    */
  ...
  /** A sequence of wrappers to apply to the resulting [[firrtl2.options.TransformLike TransformLike]] sequence. This can
  ...
  /** A method that will create a copy of this [[firrtl2.options.DependencyManager DependencyManager]], but with different
  ...
  /** Pull in all registered [[firrtl2.options.TransformLike TransformLike]] once [[firrtl2.options.TransformLike
  ...
  /** An ordering of [[firrtl2.options.TransformLike TransformLike]]s that causes the requested [[firrtl2.options.DependencyManager.targets
    * targets]] to be executed starting from the [[firrtl2.options.DependencyManager.currentState currentState]]. This ordering respects
    * prerequisites, optionalPrerequisites, optionalPrerequisiteOf, and invalidates of all constituent
    * [[firrtl2.options.TransformLike TransformLike]]s. This uses an algorithm that attempts to reduce the number of
    * re-lowerings due to invalidations. Re-lowerings are implemented as new [[firrtl2.options.DependencyManager]]s.
    *
    * @throws firrtl2.options.DependencyManagerException if a cycle exists in either the [[firrtl2.options.DependencyManager.dependencyGraph
    * dependencyGraph]] or the [[firrtl2.options.DependencyManager.invalidateGraph invalidateGraph]].
    */
  ...
  /** A version of the [[firrtl2.options.DependencyManager.transformOrder transformOrder]] that flattens the transforms of any internal
    * [[firrtl2.options.DependencyManager DependencyManager]]s.
    */
  ...
  final override def transform(annotations: A): A = {
    ...
    flattenedTransformOrder.map { t =>
      ...
      case ((a, state), t) =>
        ...
        val (timeMillis, annosx) = firrtl2.Utils.time { t.transform(a) }
  ...
  /** Get a name of some [[firrtl2.options.TransformLike TransformLike]] */
