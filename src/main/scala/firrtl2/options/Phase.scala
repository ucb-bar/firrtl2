...
/** Mix-in that makes a [[firrtl2.options.TransformLike TransformLike]] guaranteed to be an identity function on some
  * type.
  *
  * @tparam A the transformed type
  */
...
/** Mixin that defines dependencies between [[firrtl2.options.TransformLike TransformLike]]s (hereafter referred to as
  * "transforms")
  *
...
trait DependencyAPI[A <: DependencyAPI[A]] { this: TransformLike[_] =>
  ...
  /** All transforms that must run ''after'' this transform
    ...
    *
    * @see [[firrtl2.passes.CheckTypes]] for an example of an optional checking [[firrtl2.Transform]]
    *      $seqNote
    */
  @deprecated(
...
/** A mathematical transformation of an [[AnnotationSeq]].
  *
  * A [[firrtl2.options.Phase Phase]] forms one unit in the Chisel/FIRRTL Hardware Compiler Framework (HCF). The HCF is
  * built from a sequence of [[firrtl2.options.Phase Phase]]s applied to an [[AnnotationSeq]]. Note that a
  * [[firrtl2.options.Phase Phase]] may consist of multiple phases internally.
  */
trait Phase extends TransformLike[AnnotationSeq] with DependencyAPI[Phase] {

  /** The name of this [[firrtl2.options.Phase Phase]]. This will be used to generate debug/error messages or when deleting
...
/** A [[firrtl2.options.TransformLike TransformLike]] that internally ''translates'' the input type to some other type,
  * transforms the internal type, and converts back to the original type.
  *
  * This is intended to be used to insert a [[firrtl2.options.TransformLike TransformLike]] parameterized by type `B`
  * into a sequence of [[firrtl2.options.TransformLike TransformLike]]s parameterized by type `A`.
  *
  * @tparam A the type of the [[firrtl2.options.TransformLike TransformLike]]
