...
class StutteringClockTransform extends Transform {
  ...
  override def optionalPrerequisites: Seq[TransformDependency] = Seq(
    Dependency[firrtl2.passes.InlineInstances],
  ...
  override protected def execute(state: CircuitState): CircuitState = {
    ...
    val clockToEnable = clocks.map { c =>
      c -> ir.Reference(namespace.newName(c + "_en"), Utils.BoolType, firrtl2.PortKind, firrtl2.SourceFlow)
    }.toMap
  ...
  private class Context(globalClockName: String, scanResults: ScanCtx, val isPresetReg: String => Boolean) {
    val globalClock: ir.Reference = ir.Reference(globalClockName, ir.ClockType, firrtl2.PortKind, firrtl2.SourceFlow)
    // keeps track of which clock signals will be replaced by which clock enable signal
