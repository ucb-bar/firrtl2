...
import firrtl2.options.{CustomFileEmission, Dependency}
import firrtl2.passes.LowerTypes
import firrtl2.passes.MemPortUtils.memPortField
import firrtl2.stage.{FirrtlOptions, TransformManager}
...
private[firrtl2] class RtlilEmitter extends SeqTransform with Emitter {
  ...
  private class RtlilRender(
    ...
    private def emit_streams(): Unit = {
      ...
      emit(Seq("# Generated by firrtl2.RtlilEmitter (FIRRTL Version ", BuildInfo.version + ")"))
      emit(Seq("autoidx 1"))
...
private[firrtl2] class EmissionOptionMap[V <: EmissionOption](val df: V) {
...
private[firrtl2] class EmissionOptions(annotations: AnnotationSeq) {
