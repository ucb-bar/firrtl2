...
import firrtl2.{AnnotationSeq, EmitAllModulesAnnotation, EmitCircuitAnnotation}
import firrtl2.annotations.Annotation
import firrtl2.options.{Dependency, OptionsException, Phase}

/** [[firrtl2.options.Phase Phase]] that strictly validates an [[AnnotationSeq]]. The checks applied are intended to be
  * extremeley strict. Nothing is inferred or assumed to take a default value (for default value resolution see
  * [[AddDefaults]]).
  *
  * The intent of this approach is that after running this [[firrtl2.options.Phase Phase]], a user can be absolutely
  * certain that other [[firrtl2.options.Phase Phase]]s or views will succeed. See [[FirrtlStage]] for a list of
  * [[firrtl2.options.Phase Phase]] that commonly run before this.
  */
class Checks extends Phase {
  ...
  /** Determine if annotations are sane
    *
    * @param annos a sequence of [[firrtl2.annotations.Annotation Annotation]]
    * @return true if all checks pass
    * @throws firrtl2.options.OptionsException if any checks fail
    */
  def transform(annos: AnnotationSeq): AnnotationSeq = {
    ...
    annos.foreach(_ match {
      ...
      case a @ RunFirrtlTransformAnnotation(_: firrtl2.Emitter) => a +=: emitter
      case _ =>
    })
