...
import firrtl2.{AnnotationSeq, EmitAllModulesAnnotation, EmitCircuitAnnotation, Emitter, Parser}
import firrtl2.annotations.NoTargetAnnotation
import firrtl2.FileUtils
import firrtl2.options.{InputAnnotationFileAnnotation, OptionsException, Phase, StageOptions, StageUtils}
...
/** Provides compatibility methods to replicate deprecated `Driver` semantics.
  ...
  * derive an annotation file. Summarily, the [[firrtl2.options.Phase Phase]]s provided by this enable this type of
  * resolution.
  *
...
object DriverCompatibility {
  ...
  /** Determine the top name using the following precedence (highest to lowest):
    ...
    *  - Implicitly from the top module ([[firrtl2.ir.Circuit.main main]]) of a [[FirrtlCircuitAnnotation]]
    *  - Implicitly from the top module ([[firrtl2.ir.Circuit.main main]]) of a [[FirrtlSourceAnnotation]]
    *  - Implicitly from the top module ([[firrtl2.ir.Circuit.main main]]) of a [[FirrtlFileAnnotation]]
    *
  ...
  private def topName(annotations: AnnotationSeq): Option[String] =
    annotations.collectFirst { case TopNameAnnotation(n) => n }
      .orElse(annotations.collectFirst { case FirrtlCircuitAnnotation(c) => c.main }.orElse(annotations.collectFirst {
        ...
        case FirrtlFileAnnotation(f) => Parser.parse(FileUtils.getText(f)).main
      })))

  /** Determine the target directory with the following precedence (highest to lowest):
    *  - Explicitly from the user-specified [[firrtl2.options.TargetDirAnnotation TargetDirAnnotation]]
    *  - Implicitly from the default of [[firrtl2.options.StageOptions.targetDir StageOptions.targetDir]]
    *
  ...
  /** Add an implicit annotation file derived from the determined top name of the circuit if no
    * [[firrtl2.options.InputAnnotationFileAnnotation InputAnnotationFileAnnotation]] is present.
    *
    * The implicit annotation file is determined through the following complicated semantics:
    *   - If an [[firrtl2.options.InputAnnotationFileAnnotation InputAnnotationFileAnnotation]] already exists, then
    *     nothing is modified
    *   - If the derived topName (the `main` in a [[firrtl2.ir.Circuit Circuit]]) is ''discernable'' (see below) and a
    *     file called `topName.anno` (exactly, not `topName.anno.json`) exists, then this will add an
    *     [[firrtl2.options.InputAnnotationFileAnnotation InputAnnotationFileAnnotation]] using that `topName.anno`
  ...
  class AddImplicitAnnotationFile extends Phase {
    ...
    /** Try to add an [[firrtl2.options.InputAnnotationFileAnnotation InputAnnotationFileAnnotation]] implicitly specified by
      * an [[AnnotationSeq]].
      */
