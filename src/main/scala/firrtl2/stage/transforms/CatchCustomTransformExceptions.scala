...
import firrtl2.{CircuitState, CustomTransformException, Transform}
...
object CatchCustomTransformExceptions {
  ...
  private[firrtl2] def isCustomTransform(xform: Transform): Boolean = {
    ...
    Option(xform.getClass.getPackage).forall { p =>
      getTopPackage(p) != firrtl2.Parser.getClass.getPackage
    }
  }
