...
import firrtl2.options.{Dependency, HasShellOptions, OptionsException, ShellOption, Unserializable}
...
/** Indicates that this is an [[firrtl2.annotations.Annotation Annotation]] directly used in the construction of a
  * [[FirrtlOptions]] view.
  */
sealed trait FirrtlOption extends Unserializable { this: Annotation => }

/** Indicates that this [[firrtl2.annotations.Annotation Annotation]] contains information that is directly convertable
  * to a FIRRTL [[firrtl2.ir.Circuit Circuit]].
  */
sealed trait CircuitOption extends Unserializable { this: Annotation =>

  /** Convert this [[firrtl2.annotations.Annotation Annotation]] to a [[FirrtlCircuitAnnotation]]
    */
...
case class InfoModeAnnotation(modeName: String = "use") extends NoTargetAnnotation with FirrtlOption {
  ...
  /** Return the [[Parser.InfoMode]] equivalent for this [[firrtl2.annotations.Annotation Annotation]]
    *
...
object RunFirrtlTransformAnnotation extends HasShellOptions {
  ...
  private[firrtl2] def stringToEmitter(a: String): RunFirrtlTransformAnnotation = {
    val emitter = a match {
  ...
  val options = Seq(
    ...
    new ShellOption[String](
      ...
      toAnnotationSeq = _ match {
        case "lower" => Seq(RunFirrtlTransformAnnotation(new firrtl2.features.LowerCaseNames))
        case "upper" => Seq(RunFirrtlTransformAnnotation(new firrtl2.features.UpperCaseNames))
...
/** Holds a FIRRTL [[firrtl2.ir.Circuit Circuit]]
  *
  ...
  val options = Seq(
    new ShellOption[Unit](
      ...
      toAnnotationSeq = { _ =>
        ...
        firrtl2.options.StageUtils.dramaticWarning(msg)
        Seq(this)
      },
