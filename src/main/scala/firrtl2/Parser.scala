...
object Parser extends LazyLogging {
  ...
  /** Parses a file in a given filename and returns a parsed [[firrtl2.ir.Circuit Circuit]] */
  def parseFile(filename: String, infoMode: InfoMode): Circuit =
    parseCharStream(CharStreams.fromFileName(filename), infoMode)

  /** Parses a String and returns a parsed [[firrtl2.ir.Circuit Circuit]] */
  def parseString(text: String, infoMode: InfoMode): Circuit =
    parseCharStream(CharStreams.fromString(text), infoMode)

  /** Parses a org.antlr.v4.runtime.CharStream and returns a parsed [[firrtl2.ir.Circuit Circuit]] */
  ...
  /** Parse the concrete syntax of a FIRRTL [[firrtl2.ir.Circuit]], e.g.
    *   {{{
    *     """circuit Top:
    *       |  module Top:
    *       |    input x: UInt
  ...
  /** Parse the concrete syntax of a FIRRTL [[firrtl2.ir.Type]], e.g.
    *   "UInt<3>" becomes:
    *   {{{
  ...
  /** Parse the concrete syntax of a FIRRTL [[firrtl2.ir.Statement]], e.g.
    *   "wire x: UInt" becomes:
    *   {{{
  ...
  /** Parse the concrete syntax of a FIRRTL [[firrtl2.ir.Port]], e.g.
  ...
  /** Parse the concrete syntax of a FIRRTL [[firrtl2.ir.DefModule]], e.g.
    *   {{{
    *     """module Top:
    *       |  input x: UInt
    *       |  node y = x
  ...
  /** Parse the concrete syntax of a FIRRTL [[firrtl2.ir.Info]], e.g.
