...
/** Base trait for annotations that control the behavior of transforms that sub-class ManipulateNames
  ...
  *                          behavior, use a combination of a sub-class of this annotation and a [[firrtl2.transforms.NoDedupAnnotation
  * NoDedupAnnotation]].
  */
...
/** Annotation to prevent name manipulation of[[firrtl2.annotations.Target Target]]s in a transform that subclasses
...
/** Annotation to filter name manipulation to only manipulate specific [[firrtl2.annotations.Target Target]]s in a
...
case class ManipulateNamesAllowlistResultAnnotation[A <: ManipulateNames[_]](
  ...
  /** Return [[firrtl2.RenameMap RenameMap]] from old targets to new targets */
...
/** A datastructure used to do single-pass name manipulation
  ...
  * @param block   a function that returns true if a [[firrtl2.annotations.Target Target]] should not be renamed
  * @param allow   a function that returns true if a [[firrtl2.annotations.Target Target]] should be renamed
  */
...
abstract class ManipulateNames[A <: ManipulateNames[_]: ClassTag] extends Transform {
  ...
  override def prerequisites:          Seq[TransformDependency] = Seq(Dependency(firrtl2.passes.LowerTypes))
