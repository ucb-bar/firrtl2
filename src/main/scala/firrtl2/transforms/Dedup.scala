...
import firrtl2.passes.{InferTypes, MemPortUtils}
import firrtl2.Utils.{kind, splitRef, throwInternalError}
import firrtl2.annotations.transforms.DupedResult
...
/** If this [[firrtl2.annotations.Annotation Annotation]] exists in an [[firrtl2.AnnotationSeq AnnotationSeq]],
  * then the [[firrtl2.transforms.DedupModules]] transform will *NOT* be run on the circuit.
  *  - set with '--no-dedup'
  */
...
class DedupModules extends Transform {
  ...
  override def prerequisites = firrtl2.stage.Forms.Resolved
