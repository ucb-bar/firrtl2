...
import firrtl2.PrimOps.{Bits, Head, Shr, Tail}
import firrtl2.Utils.{isBitExtract, isTemp}
...
object InlineBitExtractionsTransform {
  ...
  /** Mapping from references to the [[firrtl2.ir.Expression Expression]]s that drive them */
  type Netlist = mutable.HashMap[WrappedExpression, Expression]

  /** Recursively replace [[WRef]]s with new [[firrtl2.ir.Expression Expression]]s
    *
    * @param netlist a '''mutable''' HashMap mapping references to [[firrtl2.ir.DefNode DefNode]]s to their connected
    *                [[firrtl2.ir.Expression Expression Expression]]s. It is '''not''' mutated in this function
  ...
  /** Inline bits in a Statement
    *
    * @param netlist a '''mutable''' HashMap mapping references to [[firrtl2.ir.DefNode DefNode]]s to their connected
    *                [[firrtl2.ir.Expression Expression]]s. This function '''will''' mutate it if stmt is
    *                a [[firrtl2.ir.DefNode DefNode]] with a Temporary name and a value that is a [[firrtl2.ir.PrimOp PrimOp]] Bits
