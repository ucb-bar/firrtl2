...
import firrtl2.graph.{CyclicException, DiGraph, MutableDiGraph}
import firrtl2.ir._
import firrtl2.passes.MemPortUtils
import firrtl2.{InstanceKind, PortKind, SinkFlow, SourceFlow, Utils, WInvalid}
...
class ConnectionGraph protected (val circuit: Circuit, val digraph: DiGraph[ReferenceTarget], val irLookup: IRLookup)
  ...
  /** Optionally returns the shortcut a previous BFS search may have found out of a module, starting from target
    *
    * @param target first target to find shortcut.
    * @return [[firrtl2.annotations.ReferenceTarget]] of short cut.
    */
  ...
  /** Linearizes (topologically sorts) a DAG
    *
    * @throws firrtl2.graph.CyclicException if the graph is cyclic
        ...
        val ret = pathlessEdges.flatMap {
          ...
          case localSink if enteringParentInstance(source)(localSink) =>
            ...
            if (currentStack.nonEmpty && currentStack.head.module == localSink.module) {
              ...
              bfsShortCuts(modulePort) = destinations.union(localSource)
...
object ConnectionGraph {
  ...
  /** Returns a [[firrtl2.graph.DiGraph]] of [[firrtl2.annotations.Target]] and corresponding [[IRLookup]]
  ...
  /** Within a module, given an [[firrtl2.ir.Expression]] inside a module, return a corresponding [[firrtl2.annotations.ReferenceTarget]]
    *
  ...
  private def buildCircuitGraph(circuit: Circuit): ConnectionGraph = {
    ...
    def buildInstance(m: ModuleTarget, tagger: TokenTagger, name: String, ofModule: String, tpe: Type): Unit = {
      ...
      val modulePorts = tpe.asInstanceOf[BundleType].fields.flatMap {
        // Module output
        case firrtl2.ir.Field(name, Default, tpe) => Utils.create_exps(Reference(name, tpe, PortKind, SourceFlow))
        // Module input
        case firrtl2.ir.Field(name, Flip, tpe) => Utils.create_exps(Reference(name, tpe, PortKind, SinkFlow))
    ...
    def buildStatement(m: ModuleTarget, tagger: TokenTagger)(stmt: Statement): Statement = {
      stmt match {
        ...
        /** @todo [[firrtl2.Transform.prerequisites]] ++ [[firrtl2.passes.ExpandWhensAndCheck]] */
    ...
    def buildExpression(
      ...
      /** @todo [[firrtl2.Transform.prerequisites]] ++ [[firrtl2.stage.Forms.Resolved]]. */
