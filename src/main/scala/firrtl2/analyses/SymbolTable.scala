...
import firrtl2.{InstanceKind, Kind, WDefInstance}
...
trait SymbolTable {
  ...
  def declare(d: DefMemory):   Unit = declare(d.name, MemPortUtils.memType(d), firrtl2.MemKind)
  def declare(d: DefNode):     Unit = declare(d.name, d.value.tpe, firrtl2.NodeKind)
  def declare(d: DefWire):     Unit = declare(d.name, d.tpe, firrtl2.WireKind)
  def declare(d: DefRegister): Unit = declare(d.name, d.tpe, firrtl2.RegKind)
  def declare(d: Port):        Unit = declare(d.name, d.tpe, firrtl2.PortKind)
}
...
/** Trusts the type annotation on DefInstance nodes instead of re-deriving the type from
  ...
  */
private[firrtl2] abstract class LocalSymbolTable extends SymbolTable {
...
private[firrtl2] abstract class ModuleTypesSymbolTable(moduleTypes: String => Type) extends SymbolTable {
...
private[firrtl2] trait WithSeq extends SymbolTable {
...
private[firrtl2] trait WithMap extends SymbolTable {
...
private[firrtl2] trait Symbol { def name: String; def tpe: Type; def kind: Kind }

/** only remembers the names of symbols */
private[firrtl2] class NamespaceTable extends LocalSymbolTable {
...
object SymbolTable {
  ...
  private def scanStatement(s: Statement)(implicit table: SymbolTable): Unit = s match {
    ...
    case s: IsDeclaration if s.name.nonEmpty => table.declare(s.name, UnknownType, firrtl2.UnknownKind)
