...
import firrtl2._
import firrtl2.ir._
import firrtl2.graph._
import firrtl2.Utils._
...
class InstanceGraph(c: Circuit) {
  ...
  /** An [[firrtl2.graph.EulerTour EulerTour]] representation of the [[firrtl2.graph.DiGraph DiGraph]] */
  ...
  /** Given a circuit, returns a map from module name to children
    * instance/module [[firrtl2.annotations.TargetToken]]s
    */
  @deprecated("Use InstanceKeyGraph.getChildInstances instead.", "FIRRTL 1.4")
  ...
  // Transforms an IterableOnce input into an order-preserving map
  // Iterates only once, no intermediate collections
  private def asOrderedMap[K1, K2, V](it: IterableOnce[K1], f: (K1) => (K2, V)): collection.Map[K2, V] = {
    mutable.LinkedHashMap.from(it.iterator.map { k => f(k) })
  }
