...
import firrtl2.graph.{DiGraph, EulerTour, MutableDiGraph}
import firrtl2.ir
  ...
  def collectInstances(m: ir.DefModule): Seq[InstanceKey] = m match {
    ...
    case ir.Module(_, _, _, body) => {
      ...
      def onStmt(s: ir.Statement): Unit = s match {
        case firrtl2.WDefInstance(_, name, module, _) => instances += InstanceKey(name, module)
        case ir.DefInstance(_, name, module, _)       => instances += InstanceKey(name, module)
        case _: firrtl2.WDefInstanceConnector =>
          firrtl2.Utils.throwInternalError("Expecting WDefInstance, found a WDefInstanceConnector!")
